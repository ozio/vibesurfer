...
export function buildReferenceVideoMarkup({
  ...
  return `<vibe-video id="reference-video" aria-label="Northbridge after dark" data-pacing="balanced" data-aspect-ratio="16:9"${musicIntent}>
    ...
    <div data-vibe-video-controls><button type="button" data-vibe-video-action="toggle" data-play-label="Play" data-pause-label="Pause">Play</button><button type="button" data-vibe-video-action="stop">Stop</button><label>Timeline <input type="range" data-vibe-video-seek min="0" value="0" step="100"></label><output data-vibe-video-time="current">0:00</output><span aria-hidden="true"> / </span><output data-vibe-video-time="duration">--:--</output><button type="button" data-vibe-video-action="mute">Mute</button><button type="button" data-vibe-video-action="fullscreen">Fullscreen</button></div>
  </vibe-video>`;
}
...
export function buildGenerationDebugFixture(
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Generation debug fixture</title><style>
    *{box-sizing:border-box}body{margin:0;background:#eef1f5;color:#172033;font:14px/1.45 Arial,sans-serif}.debug-page{max-width:1240px;margin:auto;padding:28px}.debug-page>header{margin-bottom:22px}.debug-page h1{margin:0 0 8px;font-size:30px}.debug-page>header p{margin:0;color:#596577}.debug-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px}.debug-card{min-width:0;padding:16px;border:1px solid #c8d0dc;border-radius:12px;background:white;box-shadow:0 3px 12px #1d2a3a12}.debug-card>header{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:14px}.debug-card code{font-size:10px;color:#667085}.debug-card svg,.debug-card img{display:block;max-width:100%;height:auto}.debug-disabled{padding:16px;border:1px dashed #a9b2bf;border-radius:8px;color:#697586;background:#f6f7f9}.debug-photo{width:100%;aspect-ratio:16/9;object-fit:cover;border-radius:8px}.debug-bars{display:flex;align-items:end;gap:8px;height:120px;padding:12px;border-left:2px solid #5d6b80;border-bottom:2px solid #5d6b80}.debug-bars i{flex:1;background:#5e5ce6;border-radius:4px 4px 0 0}.debug-flow{display:flex;align-items:center;justify-content:center;gap:9px;min-height:90px}.debug-flow span{padding:9px;border:1px solid #8390a3;border-radius:7px;background:#f8fafc}.debug-code{overflow:auto;padding:12px;border-radius:8px;background:#101722;color:#a7f3d0}.debug-map{width:100%;aspect-ratio:16/9;color:#3256a8;background:#edf3ff;border-radius:8px}.debug-avatar{width:72px;height:72px;border-radius:50%;display:grid;place-items:center;color:white;background:linear-gradient(135deg,#7257d5,#32a6a0);font-size:26px;font-weight:700}.debug-pattern{min-height:96px;padding:18px;border-radius:8px;background-color:#f4f1e8;--vibe-pattern-color:#7b715c22;--vibe-pattern-size:18px;background-image:radial-gradient(circle,var(--vibe-pattern-color) 1px,transparent 1.5px);background-size:var(--vibe-pattern-size) var(--vibe-pattern-size)}.debug-scene-art{min-height:130px;background:linear-gradient(135deg,#203652,#af7958)}[data-vibe-slideshow]>article{min-height:130px;padding:14px;border-radius:8px;background:#e9edf5}.debug-carousel{display:flex;overflow:auto;gap:10px}.debug-carousel article{min-width:70%;padding:22px;border-radius:8px;background:#edf3ff}.debug-controls,[data-vibe-video-controls]{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:10px}.debug-controls button,.debug-card>button,[data-vibe-video-controls] button{padding:7px 11px;border:1px solid #8390a3;border-radius:6px;background:#f7f8fa}[data-vibe-video-controls] label{display:flex;align-items:center;gap:6px;flex:1 1 160px}[data-vibe-video-controls] input{min-width:90px;flex:1}[data-vibe-video-caption]{display:block;margin:8px 0}[data-vibe-video-transcript]{margin-top:8px}.debug-qr{width:112px;height:112px}.debug-widget{padding:10px;border-radius:8px;background:linear-gradient(90deg,#5e5ce6 var(--vibe-progress,62%),#e5e7eb 0);color:#111;font-weight:700}.debug-motion{padding:18px;border-radius:8px;background:#edf3ff}.debug-icon{width:56px;color:#5e5ce6}.debug-script-ok{color:#147a45;font-weight:700}.debug-region{padding:12px;border:1px solid #83a1d5;border-radius:8px;background:#f4f7ff}vibe-video{display:block;overflow:hidden;border-radius:10px;background:#0a0d13;color:#f7f8fb}vibe-video>[data-vibe-scene]{min-height:260px;padding:24px;display:grid;align-content:center;gap:10px;position:relative}vibe-video>[data-vibe-scene][hidden]{display:none}vibe-video img{width:100%;height:220px;object-fit:cover;border-radius:8px}vibe-video [data-kind="split"]{grid-template-columns:1fr 1fr;align-items:center}.video-kicker{font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:#9fb2d2}.video-stat{font-size:58px}[data-vibe-narration]{color:#d3dbea}[data-vibe-video-controls]{padding:12px;background:#141a25}[data-vibe-video-caption],[data-vibe-video-status],[data-vibe-video-transcript]{margin:0;padding:8px 14px;background:#0f141d;color:#dce5f5}@media(max-width:600px){.debug-page{padding:16px}vibe-video [data-kind="split"]{grid-template-columns:1fr}}
